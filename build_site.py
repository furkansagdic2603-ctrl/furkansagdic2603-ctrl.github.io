...
for path in sorted(ROOT.glob('*/**/index.html')):
    ...
    content=f'<div class="article-head"><a class="back" href="{parent_url}">← {escape(label)}</a><div class="eyebrow">{escape(label)} · {escape(date)}</div><h1>{escape(title)}</h1><p class="reading-time">Yaklaşık {minutes} dk okuma · {format(word_count, ",").replace(",", ".")} kelime</p></div>{chapter_note}<section class="listen-panel" aria-label="Yazıyı dinle"><strong>Yazıyı dinle</strong><audio controls preload="none" hidden></audio><button class="browser-voice" type="button">Tarayıcı sesiyle dinle</button><p class="listen-status" role="status">Yapay zekâ ses kaydı varsa burada açılır.</p></section><article class="{"prose yazi-icerik docx-content" if category == "kitap-notlari" else "prose yazi-icerik"}" data-article="true" data-category="{escape(category_path, quote=True)}">{body}</article>{chapter_navigation}<div class="article-end"><a href="{parent_url}">← {escape(label)} yazıları</a></div>{comments}'
